...
$encryptPassword = $db->query("SELECT `password` FROM tbl_client_users WHERE id = $SESS_USER_ID")->fetchColumn(0);
    if (password_verify($txtCurrentPassword, $encryptPassword)) {
        $stmt = $db->prepare('UPDATE tbl_client_users SET password=?, viewPassword=? where id = ?');
        $stmt->bindParam(1, $hashPassword);
        $stmt->bindParam(2, $txtNewPassword);
