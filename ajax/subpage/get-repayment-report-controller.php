...
try {
    ...
    <table class='table table-striped table-align-left' id='repaymentReports'>
        <thead>
            <tr>
                <th>#</th>
                <th>Loan Id</th>
        ...
        $sqlPMethod = "SELECT * FROM tbl_loan_reschedule where paymentDate BETWEEN '$txtStartDate' AND '$txtEndDate' ";
        ...
        while ($row = $stmt->fetchObject()) {
            ...
            $loanOfficerId = $db->query("SELECT `loanOfficerId` FROM tbl_loan WHERE id = $LoanId")->fetchColumn(0);

            if($loanOfficerId == $selectLoanOfficer){
            $arreasReport .= "<tr>";
