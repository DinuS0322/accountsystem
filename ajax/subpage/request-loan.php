...
try{
    ...
    $requestLoan = 1;
    $aprovalStatus = 'Requested';
    $updateSavingStmt = $db->prepare('UPDATE tbl_loan SET requestLoan=?, aprovalStatus=? where id = ? ');

    $updateSavingStmt->bindParam(1, $requestLoan);
    $updateSavingStmt->bindParam(2, $aprovalStatus);
    $updateSavingStmt->bindParam(3, $id);
    $updateSavingStmt->execute();

         $activityPath = 'requestLoan - ' . $id;
